...
Dear annotator, in this assignment, your task is to annotate
sentiments in a corpus of Twitter messages.  We define
\emph{sentiments} as polar (either positive or negative) evaluative
opinions about some persons, subjects, or events.  In this assignment,
you have to mark both -- text spans denoting the opinions
(\emph{sentiments}) and text spans denoting the subjects and events
being evaluated (\emph{sentiment targets}).  Additionally, you should
also annotate the originators of the opinions (\emph{sentiment
  sources}) and lexical elements which might significantly influence
the polarity and intensity of sentiments.  The latter elements
include:
\begin{itemize}
  \item \emph{emotional expressions}, which are words or phrases that
    unequivocally possess some evaluative lexical meaning by
    themselves (examples of such expressions are words like
    \emph{hassen} (\emph{hate}), \emph{bewundern} (\emph{admire}),
    \emph{sch\"on} (\emph{nice}) etc.);
  ...
  \item \emph{intensifiers} and \emph{diminishers} (or
    ...
    respectively, increase or decrease the polar evaluative sense of
    emotional expressions.  Examples of intensifiers are words like
    \emph{sehr} (\emph{very}), \emph{besonders} (\emph{especially}),
    \emph{insbesondere} (\emph{particularly}) etc.  Diminishers are
    words like \emph{ein wenig} (\emph{a little}), \emph{ein bisschen}
    (\emph{a bit}), \emph{gewisserma\ss{}en} (\emph{to a certain
      degree}) etc.
\end{itemize}

After marking these elements, you should also specify their respective
attributes like, for example, \emph{polarity} for sentiments or
\emph{intensity} for emotional expressions.  You can find a complete
list of all elements to be marked along with the description of all
their possible attributes in Section \ref{sec:markables}.  In Section
\ref{sec:faq}, we also provide some annotation examples and answer
questions which caused special difficulties during the previous
annotation runs.
...
After downloading this file, unpack the received archive, change to
the newly created directory \texttt{1.13.003/MMAX2} in your terminal
shell and execute the following commands: \code{chmod u+x
...
\texttt{*.mmax} files found there.  The chosen annotation project
should then be loaded into your \texttt{MMAX2} editor.
...
\subsection{Format and Terminology}

Throughout this document, we will rely on the following conventions
regarding the format and terminology.

We will specify shell commands in gray boxes in \texttt{typesetting}
font as shown in the example below: \code{echo 'Hello world!'}  The
\texttt{typesetting} font will also be used for literal mentions of
markable types, names of the attributes, as well as for literal
mentions of directory paths and executable commands.

Examples of single words and phrases will be given in \textit{italics}
with their respective English translation provided in parentheses.

Examples of sentence annotations will be given using regular
font. Text enclosed in markables will be \emph{emphasized}.  We will
mark the boundaries of the markables using square brackets.  The type
of the markables will be given as subscript after the closing right
bracket; their optional attributes will follow the type, separated
from it by a colon.  We also provide English translations of German
examples in parentheses, as shown in Example \ref{exmpl:1}:
\begin{exmp}
...
In this document, we won't make a difference between the terms
\emph{opinion} and \emph{sentiment} and will use them interchangeably
unless the latter refers to the literal name of the markable
\textt{sentiment}.
