...
\ref{qstn:condition}, and \ref{qstn:irrealis} in the \nameref{sec:faq}
...
\item\textbf{Q: Should I annotate sentiments in conditional
  ...
  If conditional sentence describes some external condition, under
  which the author would like or dislike a particular thing or event,
  then you should annotate the (dis-)liked thing as a \texttt{target}
  of \texttt{sentiment} (cf. Examples \ref{exmp:cond1},
  \ref{exmp:cond2}, and \ref{exmp:cond3}).

  \begin{exmp}
    \sentiment[polarity=positive]{Wenn es regnet, \target{mag ich es
        immer, vor dem Fenster zu sitzen}.}

    (\sentiment[polarity=positive]{\target{I always like to sit in
  ...
  \begin{exmp}
    ...
    (\sentiment[polarity=positive]{If the weather was better,
      \target{I would like to go jogging}.})\label{exmp:cond2}
  \end{exmp}

  \begin{exmp}
    \sentiment[polarity=positive]{Wenn ich gesund w\"are,
      \target{w\"urde ich gern mit euch campen}.}

    (\sentiment[polarity=positive]{If I wasn't sick, \target{I would
        gladly go camping with you}.})\label{exmp:cond3}
  \end{exmp}

  \begin{exmp}
    \sentiment[polarity=positive]{Selbst wenn es keine Alternative
      g\"abe, w\"urde mir \target{dieses Auto} nicht gefallen.}

    (\sentiment[polarity=positive]{Even if there was no other
      alternative, I would not like \target{this
        car}.})\label{exmp:cond4}
  \end{exmp}

%% 3 If this car consumed less fuel, I would definitely buy it -- + <this car consumed less fuel>;
%% 3 If this car had a removable windshield, I would definitely buy it -- + <this car consumed less fuel>;
