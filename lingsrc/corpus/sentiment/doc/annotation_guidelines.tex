...
elements, we have listed the most obvious differences between these
two classes:
\begin{itemize}
...
the polarity and the intensity of expressed attitudes.  After that,
you should annotate subjects and events which are being evaluated and
mark them as \texttt{target}s.  The holders of opinions should be
annotated as \texttt{source}s.  Both, \texttt{source}s and
...
\item\textbf{Q: Should I annotate sentiments in questions?}\label{qstn:interrogative}
  ...
  \textbf{A:} It primarily depends on the type of question.  You
  ...
  \begin{itemize}
    \item If it is a \textit{yes-no-question} or \textit{wh-question},
      ...
      not, then you should not annotate any sentiment relation in this
      sentence.  In Example \ref{exm:sentiment-question1}, for
...
\item\textbf{Q: Should I consider as sentiments irrealis
  sentences?}\label{qstn:irrealis}

  If an irrealis sentence expresses a wish about a particular target,
  then you should mark the whole wish as \texttt{sentiment}, annotate
  the wished property as \texttt{target}, and set the
  \texttt{polarity} value of the \texttt{sentiment} markable to
  \texttt{positive}, if the target property or object is desired, and
  to \texttt{negative} otherwise (cf. Example \ref{exmp:irrealis-1}).
  \begin{exmp}
    ...
    (\sentiment[polarity=positive]{I \emoexpression{wish}, \target{the
        reaction of German soccer fans would be more tamed}.})
  ...
  \texttt{sentiment} markable with a negative polarity.
  \begin{exmp}
  ...
  \end{exmp}
  Such annotation reflects the fact, that the author is unsatisfied
  with the current state of a target objects, but she would be glad if
  that state would change.


\item\textbf{Q: Should I annotate wishes as sentiments?}

  \textbf{A:} Usually, yes.  If someone expresses a wish to get or to
  do something, then this person typically also has a positive
  attitude to the desired object or activity (cf. Example
  \ref{exmp:desire}).
...
\item\textbf{Q: Should I annotate sentiments in recommendations?}
  ...
  determine the target of this opinion, then you should annotate it as
  \texttt{sentiment}.
...
\item\textbf{Q: Should I mark sentiments in insults?}
  ...
  sentence \textit{Du bist ein Idiot! (You are an idiot!)}, \textit{Du
    (You)} is the target of a negative sentiment.  On the other hand,
  curses like \textit{Idiot!  (Idiot!)} do not have any explicit
  targets and, therefore, should not be annotated as
  \texttt{sentiment}s according to our definition.
...
\item\textbf{Q: Should I annotate as sentiments cases like
  ...
  her job and it would not be necessarily true that she had any
  personal attitude to the defendant.  On the contrary, if I accused
  someone of mean tricks, it would usually be my subjective opinion.
  The same is true for support feelings: if a person supports
  someone's opinion, she is usually judging positively about it.
...
\item\textbf{Q: How should I determine the intensity of a comparison?}
  ...
  a strong emotional evaluation, the \texttt{intensity} attribute of
  that \texttt{sentiment} should be set to \texttt{high}.  If, on the
  other hand, the sentence rather makes an objective statement of
  facts, then you should marked the \texttt{intensity} of such
  \texttt{sentiment} (if any should be extracted) to \texttt{medium}
  or \texttt{weak} value.

  For example, in sentence \textit{this lousy Telekom waaaaaay less
    reliable than O2}, the strength and the stylistic expressiveness
  of the sentence are much higher than in sentence \textit{Telekom has
    a less reliable connection than 02}.  Consequently, we should set
  the value of the \texttt{intensity} attribute in the former sentence
  to \texttt{strong} and in the latter case to \texttt{medium}.

\item\textbf{Q: It is said that we should disregard the negations when
